...
public class TlivFramerTemplateTest {

    @Test
    public void testLoadTemplate() throws Exception {
        final var template = TlivFramerTemplate.load();
        final var ies = CodeGen.loadInfoElementMetaData();
        final String rendered = template.render(ClassNameConverter.defaultConverter(), ies);
